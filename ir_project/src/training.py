...
class DataGenerator(tf.keras.utils.Sequence):
    def __init__(self, x_data, y_data, batch_size):
        self.x_context, self.x_context_exact_match, self.x_question = x_data
        self.y = y_data
    ...
    def __getitem__(self, idx):
        batch_x = (self.x_context[self.batch_idx[idx]], self.x_context_exact_match[self.batch_idx[idx]], self.x_question[self.batch_idx[idx]])
        batch_y = self.y[self.batch_idx[idx]]
        return batch_x, batch_y


def training(model, x_train_question, x_train_context, x_train_context_exact_match, y_train, x_val_question, x_val_context, x_val_context_exact_match, y_val):
    training_generator = DataGenerator((x_train_context, x_train_context_exact_match, x_train_question), (y_train), BATCH_SIZE)
    validation_generator = DataGenerator((x_val_context, x_val_context_exact_match, x_val_question), (y_val), BATCH_SIZE)
...
def load_predict(x_val_context, x_val_context_exact_match, x_val_question, y_val):
